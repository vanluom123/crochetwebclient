...
public class WebClientServiceImpl implements WebClientService {
    ...
    private <TRequest, TResponse> Mono<TResponse> executeWithBody(String uri, HttpMethod httpMethod, TRequest request, Class<TResponse> clazz, Consumer<HttpHeaders> headers) {
    ...
    private <TRequest, TResponse> Mono<TResponse> executeWithBody(String uri, HttpMethod httpMethod, TRequest request, Class<TResponse> clazz) {
        WebClient.RequestBodySpec bodySpec = switch (httpMethod) {
            case POST -> client().post().uri(uri);
            case PUT -> client().put().uri(uri);
            default -> throw new IllegalArgumentException("Unsupported request type: " + httpMethod);
    ...
    @Override
    public <TRequest, TResponse> Mono<TResponse> invokeApi(String uri, HttpMethod httpMethod, TRequest request, Class<TResponse> clazz, Consumer<HttpHeaders> headers) {
        return executeWithBody(uri, httpMethod, request, clazz, headers);
    }

    @Override
    public <TResponse> Mono<TResponse> invokeApi(String uri, HttpMethod httpMethod, Class<TResponse> clazz, Consumer<HttpHeaders> headers) {
        return executeWithoutBody(uri, httpMethod, clazz, headers);
    }

    @Override
    public <TResponse> Mono<TResponse> invokeApi(String uri, HttpMethod httpMethod, Class<TResponse> clazz) {
        return executeWithoutBody(uri, httpMethod, clazz);
    }

    @Override
    public <TRequest, TResponse> Mono<TResponse> invokeApi(String uri, HttpMethod httpMethod, TRequest request, Class<TResponse> clazz) {
        return executeWithBody(uri, httpMethod, request, clazz);
    }

    //------------------Async method--------------------------
    @Override
    public <TRequest, TResponse> CompletableFuture<TResponse> invokeApiAsync(String uri, HttpMethod httpMethod, TRequest request, Class<TResponse> clazz, Consumer<HttpHeaders> headers) {
        return toCompletableFuture(executeWithBody(uri, httpMethod, request, clazz, headers));
    }

    @Override
    public <TResponse> CompletableFuture<TResponse> invokeApiAsync(String uri, HttpMethod httpMethod, Class<TResponse> clazz, Consumer<HttpHeaders> headers) {
        return toCompletableFuture(executeWithoutBody(uri, httpMethod, clazz, headers));
    }

    @Override
    public <TResponse> CompletableFuture<TResponse> invokeApiAsync(String uri, HttpMethod httpMethod, Class<TResponse> clazz) {
        return toCompletableFuture(executeWithoutBody(uri, httpMethod, clazz));
    }

    @Override
    public <TRequest, TResponse> CompletableFuture<TResponse> invokeApiAsync(String uri, HttpMethod httpMethod, TRequest request, Class<TResponse> clazz) {
